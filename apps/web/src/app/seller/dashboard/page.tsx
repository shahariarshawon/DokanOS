...
import { formatPrice, formatDate, getCategoryName, getStoreName } from '@/lib/utils';
...
export default function SellerDashboardPage() {
  ...
  const [newProductPrimaryImage, setNewProductPrimaryImage] = useState<string>(
    'https://images.unsplash.com/photo-1546868871-7041f2a55e12?w=800&auto=format&fit=crop&q=80',
  );
        ...
        try {
          ...
          if (stored) {
            ...
            const initialDemoOrders = [
              ...
              {
                id: 'DOK-2026-881240',
                orderNumber: 'DOK-2026-881240',
                placedAt: new Date(Date.now() - 3600000 * 4).toISOString(),
                status: 'PENDING',
                subtotal: 999.0,
                taxAmount: 49.95,
                shippingAmount: 0.0,
                ...
                items: [
                  {
                    ...
                    productId: 'prod-001',
                    productTitle: 'iPhone 15 Pro Titanium',
                    variantTitle: 'Black Titanium / 128GB',
                    productSku: 'IPH15P-BLK-128',
                    primaryImage:
                      'https://images.unsplash.com/photo-1695048133142-1a20484d2569?w=800&auto=format&fit=crop&q=80',
                    unitPrice: 999.0,
                    quantity: 1,
            ...
            setSellerOrders(initialDemoOrders);
            localStorage.setItem('dokanos_orders', JSON.stringify(initialDemoOrders));
          }
        } catch {
          // ignore
        }
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    const newProd: Product = {
      ...
      storeRating: 5.0,
      badge: 'New Arrival',
      primaryImage: newProductPrimaryImage || newProductImages[0] || productImageUrl,
      images:
        newProductImages.length > 0
          ? newProductImages
          : [newProductPrimaryImage || productImageUrl],
      variants: [
        {
          id: `var-${Date.now()}-1`,
  ...
  const handleUpdateOrderStatus = async (orderId: string, nextStatus: string) => {
    ...
    // Auto-adjust inventory stock if an order is CANCELLED (restock items back)
    if (nextStatus === 'CANCELLED' && prevStatus !== 'CANCELLED' && targetOrder?.items) {
        ...
        if (item.productId) {
          try {
            await adjustInventoryStock({
              ...
              note: `Restocked after Order #${orderId} was CANCELLED`,
            });
          } catch {
            // ignore
          }
    ...
    // Dispatch real-time system notification for this order update
    dispatchSystemNotification({
      type: 'ORDER_STATUS',
      title: `Order #${orderId} Update`,
      body: `Status changed to ${nextStatus}. Fulfillment tracking pipeline updated.`,
    ...
    return (
      <div className="min-h-screen flex flex-col bg-zinc-50">
        ...
        <main className="flex-1 flex items-center justify-center p-6">
          <div className="max-w-md w-full rounded-2xl border border-zinc-200 bg-white p-8 text-center shadow-xl">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-amber-50 text-amber-600 mb-4">
              <Store className="w-6 h-6" />
            </div>
            <h2 className="text-lg font-bold text-zinc-900">Merchant Account Required</h2>
            <p className="mt-2 text-xs text-zinc-500 leading-relaxed">
              You are currently signed in with a customer account. To manage products and store
              settings, please upgrade to a merchant plan or visit your account portal.
            </p>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-zinc-50/50">
      ...
      <main className="flex-1 mx-auto max-w-7xl w-full px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        {role === 'ADMIN' && (
          <div className="rounded-xl border border-purple-200 bg-purple-50/70 p-3 flex items-center justify-between text-xs text-purple-900">
            <div className="flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-purple-600" />
              <span>Signed in as Administrator • Viewing Merchant Console</span>
            </div>
            <Link
              href="/admin/dashboard"
              className="font-bold underline hover:text-purple-700 shrink-0"
            >
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-zinc-900">
                      SaaS Recurring Revenue & Subscriptions
                    </h3>
                          ...
                          <tr key={prod.id} className="hover:bg-zinc-50/50 transition-colors">
                            ...
                            <td className="py-3.5 px-4">
                              <div className="flex items-center gap-2">
                                ...
                                <div className="inline-flex items-center rounded-md border border-zinc-200 bg-white shadow-2xs overflow-hidden">
                                  <button
                                    type="button"
                                    onClick={async () => {
                                      ...
                                      await adjustInventoryStock({
                                        ...
                                        type: 'ADJUSTMENT',
                                        quantity: -1,
                                        note: 'Quick stock decrease',
                                      });
                                      showToast(
                                        `Decreased stock for "${prod.title}" to ${prod.stock - 1}`,
                                      );
                                      await loadData();
                                    }}
                                    disabled={prod.stock <= 0}
                                    title="Decrease stock by 1"
                                    className="px-1.5 py-0.5 text-zinc-500 hover:text-rose-600 hover:bg-zinc-50 disabled:opacity-40 transition-colors text-xs font-bold"
                                  >
                                    -
                                  </button>
                                  ...
                                  <button
                                    ...
                                    onClick={async () => {
                                      await adjustInventoryStock({
                                        ...
                                        type: 'RESTOCK',
                                        quantity: 1,
                                        note: 'Quick stock increase',
                                      });
                                      showToast(
                                        `Increased stock for "${prod.title}" to ${prod.stock + 1}`,
                                      );
                                      await loadData();
                                    }}
                                    title="Increase stock by 1"
                                    className="px-1.5 py-0.5 text-zinc-500 hover:text-emerald-600 hover:bg-zinc-50 transition-colors text-xs font-bold"
                                  >
                                    +
                                  </button>
                                </div>
                            ...
                            <td className="py-3.5 px-4 text-right">
                              <div className="flex items-center justify-end gap-1.5">
                                ...
                                <button
                                  ...
                                  className="p-1.5 rounded-md hover:bg-indigo-50 text-zinc-500 hover:text-indigo-600 transition-colors"
                  ...
                  <div className="space-y-4">
                    {sellerOrders.map((order) => (
                      ...
                      >
                        ...
                        {/* Seller Order Actions & Full Lifecycle Status Controls */}
                        <div className="pt-3 border-t border-zinc-100 flex flex-wrap items-center justify-between gap-3 text-xs">
                          {/* Current Status Pill with Icon */}
                          <div className="flex items-center gap-2">
                            <span className="text-zinc-500 font-medium">Status:</span>
                            <span
                              className={`rounded-full px-2.5 py-0.5 text-[11px] font-bold inline-flex items-center gap-1.5 ${
                                order.status === 'DELIVERED'
                                  ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                                  : order.status === 'SHIPPED'
                                    ? 'bg-indigo-50 text-indigo-700 border border-indigo-200'
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <select
                              ...
                              className="rounded-lg border border-zinc-200 bg-white px-2.5 py-1 text-xs text-zinc-800 font-medium outline-none focus:border-indigo-600 cursor-pointer shadow-2xs"
                            ...
                            {/* Contextual primary advance buttons */}
                            {order.status === 'PENDING' && (
                              <button
                                type="button"
                                onClick={() => handleUpdateOrderStatus(order.id, 'CONFIRMED')}
                                className="rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-3 py-1 text-xs shadow-2xs transition-colors"
                              >
                                Confirm Order
                              </button>
                            )}
                            ...
                            {(order.status === 'CONFIRMED' || order.status === 'PAID') && (
                              <button
                                ...
                                className="rounded-lg bg-zinc-900 hover:bg-zinc-800 text-white font-semibold px-3 py-1 text-xs shadow-2xs transition-colors"
                              >
                                Start Processing
                              </button>
                            )}
                            ...
                            {order.status === 'SHIPPED' && (
                              <button
                                ...
                                className="rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-3 py-1 text-xs shadow-2xs transition-colors"
                              >
                            ...
                            {order.status !== 'CANCELLED' && order.status !== 'DELIVERED' && (
                              <button
                                ...
                                onClick={() => {
                                  if (
                                    confirm(`Cancel Order #${order.orderNumber} and restock items?`)
                                  ) {
                                    handleUpdateOrderStatus(order.id, 'CANCELLED');
                                  }
                                }}
                                className="rounded-lg border border-rose-200 hover:bg-rose-50 text-rose-600 font-semibold px-2.5 py-1 text-xs transition-colors"
                              >
                                Cancel Order
                              </button>
                            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="w-full max-w-lg rounded-2xl border border-zinc-200 bg-white p-6 shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateProduct} className="mt-4 space-y-4 text-xs">
              ...
              {/* Product Image Management Studio */}
              <div>
                <ProductImageManager
                  images={newProductImages}
                  primaryImage={newProductPrimaryImage}
                  onChange={(imgs, prim) => {
                    setNewProductImages(imgs);
                    setNewProductPrimaryImage(prim);
                    setProductImageUrl(prim);
                  }}
                  maxImages={8}
                />
              </div>
      ...
      <ProductEditModal
        ...
        onSaved={(updated) => {
          ...
          showToast(`Saved updates for product "${updated.title}"`);
        }}
      />
