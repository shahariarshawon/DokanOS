...
from modules.cache.cache_manager import embedding_cache

logger = logging.getLogger("dokanos.ai.embedding_service")

EMBEDDING_DIM = 1536

class EmbeddingService:
    """
    Independent vector embedding service supporting multiple providers with automatic failover:
    1. Memory Cache (O(1) retrieval for repeated queries)
    2. OpenAI (text-embedding-3-small, 1536 dims)
    3. Gemini (text-embedding-004, 1536 dims)
    4. Deterministic Semantic Hash Projection (Offline fallback, 1536 dims, L2 normalized)
    """
    ...
    async def get_embedding(self, text: str, use_cache: bool = True) -> Tuple[List[float], str]:
        """
        Generate 1536-dimensional vector embedding for input text.
        Returns: (embedding_vector, provider_name)
        """
        clean_text = text.strip()
        if not clean_text:
            return [0.0] * EMBEDDING_DIM, "empty_fallback"

        # 1. Check in-memory TTL cache
        cache_key = f"emb:{clean_text}"
        if use_cache:
            cached_vector = embedding_cache.get(cache_key)
            if cached_vector:
                return cached_vector, "cache"

        # 2. Attempt OpenAI if key configured
        if self.openai_key:
            try:
                vec = await self._get_openai_embedding(clean_text)
                if use_cache:
                    embedding_cache.set(cache_key, vec, ttl_seconds=86400)
                return vec, "openai"
            except Exception as e:
                logger.warning(f"OpenAI embedding call failed, trying next provider: {e}")

        # 3. Attempt Gemini if key configured
        if self.gemini_key:
            try:
                vec = await self._get_gemini_embedding(clean_text)
                if use_cache:
                    embedding_cache.set(cache_key, vec, ttl_seconds=86400)
                return vec, "gemini"
            except Exception as e:
        ...
        # 4. Fallback: Deterministic semantic token vector
        vec = self._generate_fallback_embedding(clean_text)
        if use_cache:
            embedding_cache.set(cache_key, vec, ttl_seconds=86400)
        return vec, "semantic_projection_fallback"

    async def get_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        """Generates embeddings for a batch of strings."""
        results = []
        for text in texts:
            vec, _ = await self.get_embedding(text)
            results.append(vec)
        return results
    ...
    def _generate_fallback_embedding(self, text: str) -> List[float]:
        ...
        # Generate tokens: unigrams + adjacent bigrams
        tokens = list(words)
